...
SYSTEM_RULES = (
    ...
    "When the user asks the question, reply with exactly one paragraph. "
...
def ask_once(session, seed=None):
    ...
    # This is the *raw* content from the LLM (could be JSON + extra text, etc.)
    content = (data.get("message", {}) or {}).get("content", "")

    # Optional: print raw output so you can see the full sentence in the console
    print("\n--- RAW MODEL OUTPUT ---")
    print(content)
    print("------------------------\n")

    # Prefer JSON, then fallback to text
    try:
    ...
    # Return both the cleaned adjective and the raw content
    return word, content

def main():
    sess = requests.Session()
    adjectives = []
    raw_outputs = []   # <--- NEW: store full sentences / raw LLM content
    attempts = 0
    ...
    while len(adjectives) < N_TRIALS and attempts < MAX_ATTEMPTS:
        ...
        # CHANGED: get both the adjective and the full output
        w, full = ask_once(sess, seed=seed)
        ...
        adjectives.append(w)
        raw_outputs.append(full)   # keep full content aligned with adjective
    ...
    df = pd.DataFrame({
        ...
        "raw_output": raw_outputs,   # <--- NEW: column with full model output
    })
    df.to_csv("adjectives_raw_with_full_output.csv", index=False)

    # Frequencies (same as before, based only on adjectives)
    ...
    # Word cloud (unchanged, still based on adjectives)
    if freq:
    ...
    print("\nSaved:")
    print("  - adjectives_raw_with_full_output.csv  (adjective + raw LLM text)")
